...
function list_kernel() {
    echo -e "\r$green[+]$re Listing kernels\n"
...
function delete_kernel() {
    ...
    for list in $(cat $tmp_list_kernel_path); do
        if [ $args2 == $list ]; then
            ...
            clear 
            echo -e "\r$green[+] He was removed:$re $args2"
            rm -r --force $tmp_list_kernel_path
            exit 0
        fi
    done

    echo -e "\r$red[!] Kernel not found, possibly does not exist or has been uninstalled $re"
    exit 1
}
...
function grep_kenel_list() {
    ...
    echo -e "\r$red\r[!] This kernel $green $args2$re $red no exits
...
function main() {
    use="\n
    \r[?] for more information use $green--help $re or $green -h $re"
    ...
    if [ $args1 == "-v" -o $args1 == "--version" ]; then
        ...
        \rWritten by:$green Jakepy perdomo$re"
